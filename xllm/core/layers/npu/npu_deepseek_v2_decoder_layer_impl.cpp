...
void NpuDeepseekV2DecoderLayerImpl::init_atb_tensors() {
  ...
  for (size_t i = 0; i < weight_slices_.size(); ++i) {
    LOG(INFO) << "index: " << i << " size:" << at_weight_tensors_[i].sizes()
              << "layer_id " << layer_id_;
  }
  ...
  if (layer_id_ >= prefill_param_.firstKDenseReplace) {
    at_weight_tensors_[IN_MLP_GATEUP_WEIGHT_EXPERT] =
        at_npu::native::npu_format_cast(
            at_weight_tensors_[IN_MLP_GATEUP_WEIGHT_EXPERT], 29);
#if defined(USE_A3)
    at_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT] =
        at_npu::native::npu_format_cast(
            at_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT], 29);
#else
    if (decode_param_.isBF16) {
      at_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT] =
          at_npu::native::npu_format_cast(
      ...
      at_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT] =
          at_npu::native::npu_format_cast(
              at_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT], 2);
    }
#endif
  }
