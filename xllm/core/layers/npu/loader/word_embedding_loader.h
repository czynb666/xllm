...
class WordEmbeddingLoader : public BaseLoader {
 public:
  explicit WordEmbeddingLoader(uint64_t weight_count,
                               const xllm::ParallelArgs& parallel_args);
  ~WordEmbeddingLoader() = default;
  void load_state_dict(const StateDict& state_dict) override;
  void verify_loaded_weights() const override;
  void merge_loaded_weights() override;

 private:
  void LoadWeights(const StateDict& state_dict,
                   const std::vector<std::pair<int, std::string>>& mapping,
                   const std::map<int, int>& shard_map);
};
