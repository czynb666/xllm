...
void DeekseekV2DecoderManualLoader::initialize_tensors(
  ...
  torch::TensorOptions cpu_options = options.device(at::kCPU);
...
void DeekseekV2DecoderManualLoader::merge_experts_weights() {
  ...
  at_host_weight_tensors_[IN_MLP_GATEUP_WEIGHT_EXPERT] =
      at_host_weight_tensors_[IN_MLP_GATEUP_WEIGHT_EXPERT].to(at::kCPU);
  ...
  if (decode_isBF16_ && false) {
    torch::Tensor mlp_down_weight =
        merge_experts_weights(experts_weights_["down_proj.weight"],
                              ...
                              /*transpose=*/true);
    at_host_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT] =
        at_npu::native::npu_format_cast(mlp_down_weight, 29).to(at::kCPU);
  } else {
    torch::Tensor mlp_down_weight =
        merge_experts_weights(experts_weights_["down_proj.weight"],
                              at::kCPU,
...
void DeekseekV2DecoderManualLoader::merge_host_at_weights() {
  ...
  at_host_weight_tensors_[IN_Q_PROJ_A_WEIGHT] =
      at_npu::native::npu_format_cast(
          at_host_weight_tensors_[IN_Q_PROJ_A_WEIGHT].to(device_), 29)
          .to(at::kCPU);
  at_host_weight_tensors_[IN_Q_PROJ_B_WEIGHT] =
      at_npu::native::npu_format_cast(
          at_host_weight_tensors_[IN_Q_PROJ_B_WEIGHT].to(device_), 29)
          .to(at::kCPU);
